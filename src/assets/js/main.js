...
$(function () {
    ...
    $('#clock2').countdown('2020/10/10').on('update.countdown', function (event) {
        var _DateInput = '' +
            '<div><span>%-d</span> Day%!d</div>' +
            '<div><span>%H</span> Hours</div>' +
            '<div><span>%M</span> Minutes</div>' +
    ...
    //flip map
    const _txt = '<i class="fa fa-map-marker-alt"></i>VIEW MAP',
    ...
    $('.o-contact-info').on('click', '.btn', function (e) {
        ...
        $(".o-contact-info .btn.btn-small").toggleText(_txt, _alterTxt);
